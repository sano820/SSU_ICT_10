...
class AgentState(TypedDict):
    ...
    streaming_route: str # 라우터가 생성하는 미리보기 메시지
...
def user_profiling_node(state: AgentState) -> Dict[str, Any]:
    ...
    **지시사항:**
    - 사용자가 자신의 정보가 잘 입력되었는지 확인할 수 있게 안내하는 문구를 넣어주세요.
    - 메시지 마지막에는, 이 정보를 바탕으로 심층 분석을 시작하겠다는 내용을 포함시켜주세요.
    - 메시지에 너무 오버하거나 과하게 친절하게 답변하지 마세요
    ...
    confirmation_chain = ChatPromptTemplate.from_template(confirmation_prompt_template) | llm | StrOutputParser()

    # 1. LLM을 통해 기본 확인 메시지를 생성합니다.
    llm_generated_message = confirmation_chain.invoke({
        "profile_summary": profile_summary_for_display
    })

    # 2. [수정] 고정된 안내 문구를 생성된 메시지 뒤에 추가합니다.
    ...
    return {
        ...
        "streaming_user_profile": final_confirmation_message # <-- 최종 메시지로 교체
    }
...
def analyze_interviews_node(state: AgentState) -> Dict[str, Any]:
    ...
    combined_interviews = interviews_web_results
...
def combine_domestic_analysis_node(state: AgentState) -> Dict[str, Any]:
    """병렬 분석 결과를 종합하고 최종 키워드를 추출합니다."""
    print("-> [취합] 병렬 분석 결과 종합 및 키워드 추출 중...")

    # 1. 병렬 노드들의 결과물을 state에서 가져오기
    ...
    llm = state.get("llm_analytical", state["llm"])
    ...
    prompt_variable_inputs = {"market_analysis_json": combined_analysis_str}
...
def analyze_tech_trends_node(state: AgentState) -> Dict[str, Any]:
    ...
    # ... (기존 2a. 기술 트렌드 (Tavily) 데이터 수집 로직)
    # LLM으로 검색어 생성 -> Tavily로 웹 검색
    # --- 1. 사전 준비 ---
...
def analyze_leaders_vision_node(state: AgentState) -> Dict[str, Any]:
    ...
    llm = state.get("llm_creative", state.get("llm"))
    ...
    try:
        # --- 2. LLM으로 분석할 주요 인물 리스트 생성 (기존과 유사) ---
        main_company = target_companies[0]
        ...
        print(f"-> 분석할 주요 인물: {key_figures_list}")

        # --- 3. [수정] Tavily를 사용하여 각 인물에 대한 인터뷰/강연 검색 ---
        search_queries = [f'"{figure_name}" interview on {target_job_title} future vision' for figure_name in key_figures_list[:3]] # 최대 3명에 대해 검색
        ...
        ---
        """
        summary_chain = ChatPromptTemplate.from_template(summary_prompt_template) | llm | StrOutputParser()
...
def combine_global_trends_node(state: AgentState) -> Dict[str, Any]:
    """병렬 분석된 글로벌 트렌드 Raw 데이터를 종합하여 최종 분석을 수행합니다."""
    print("-> [취합] 글로벌 트렌드 종합 및 최종 분석 중...")
    ...
    return {"global_trends": global_trends_analysis}
...
# (필요한 다른 클래스 및 함수, PROMPTS 딕셔너리가 이미 정의되어 있다고 가정합니다.)
...
def recommend_learning_node(state: AgentState) -> dict:
    ...
    """
    search_keywords = (llm | JsonOutputParser()).invoke(keyword_generation_prompt)
    ...
    # 구조화된 추천 결과를 LLM에 전달할 문자열로 변환
    recommendations_str = json.dumps(learning_recommendations, ensure_ascii=False, indent=2)

    # prompts.py에서 보고서 생성용 프롬프트를 가져옵니다.
    final_report_prompt = RECOMMEND_LEARNING["study_final_report"]

    report_chain = ChatPromptTemplate.from_template(final_report_prompt) | llm | StrOutputParser()
    
    streaming_report = report_chain.invoke({
...
def recommend_storytelling_node(state: AgentState) -> dict:
    ...
    recommendation_prompt_template = "" # 프롬프트 템플릿 변수 초기화

    # --- 2. [수정] 강점 유무에 따른 분기 처리 ---
    if strengths:
        # --- 2-A. 강점이 있을 경우: 강점 활용 로직 (기존 로직) ---
        print("-> 분석된 강점을 활용하는 방향으로 스토리텔링을 추천합니다.")
        
        # Tavily로 컨텍스트 검색
        search_keywords = [f"{opp} latest trends" for opp in opportunities[:2]]
        ...
        # --- 2-B. 강점이 없을 경우: 경험 생성 제안 로직 (신규 로직) ---
